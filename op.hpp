...
struct ApplyUnaryOp {
    using type = typename Prepend<typename List::type, typename ApplyUnaryOp<typename List::next, F, f>::type, f(
            List::elem)>::type;
};
...
struct ApplyBinaryOpVec {
    using type = typename Prepend<T,
            typename ApplyBinaryOpVec<T, typename List::next, b, F, f>::type, f(List::elem, b)>::type;
};
